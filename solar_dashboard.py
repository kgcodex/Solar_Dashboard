...
def generate_hourly_data(hours=24):
    ...
    std_dev = 3.5
    max_prod = 6.5 # kW
    # Gaussian-like curve for production
    production = max_prod * np.exp(-((np.arange(hours) - peak_hour)**2 / (2 * std_dev**2)))
    production[production < 0.1] = 0 # Ensure zero production at night
    production += np.random.rand(hours) * 0.3 # Add some noise
    production = np.round(production, 2)

    # Battery Level (example: discharges overnight, charges during day)
    battery = np.zeros(hours)
    min_charge = 20 # %
    max_charge = 95 # %
    charge_rate = 10 # % per hour during peak
    discharge_rate = 3 # % per hour

    current_charge = 40 # Start at 40%
    for i in range(hours):
        # Discharge slowly if no production and above min
        if production[i] < 0.5 and current_charge > min_charge:
            current_charge -= discharge_rate + np.random.rand() * 1
        # Charge if production is significant and below max
        elif production[i] > 2.0 and current_charge < max_charge:
             # Simple charge logic: charge faster during higher production
...
means = today_data.mean() # Shape (2,) Pandas Series ['Production (kW)', 'Battery Level (%)']

# --- Week Data ---
num_days_week = 7
scaling_factors_week = 1 + (np.random.rand(num_days_week) - 0.5) * 0.2 # Shape (7,) NumPy array
...
# --- Month Data ---
num_days_month = 30
scaling_factors_month = 1 + (np.random.rand(num_days_month) - 0.5) * 0.3 # Shape (30,) NumPy array
...
current_time_index = np.random.randint(8, 18) # Pick a random hour during daytime for snapshot
...
current_efficiency = np.round(85 + np.random.rand() * 10, 1) # % (Keep this random for now)

# --- Simple Flow Calculation ---
home_consumption = np.round(1.5 + np.random.rand() * 2, 2) # kW (Keep random for demo)
...
# --- Header ---
header_cols = st.columns([1, 4]) # Give more space to title
with header_cols[0]:
    # Removed Lottie, using only emoji now
...
col1, col2, col3 = st.columns([2, 2, 1.5]) # Adjust column ratios as needed
...
with col1:
    ...
    # Timeframe Tabs for Production Chart
    ...
    with bat_cols[0]:
         # Removed Lottie, using only markdown/emoji
    ...
    with bat_cols[1]:
        ...
        st.progress(current_battery_level) # Visual progress bar

    # Battery Level Chart (Today)
...
with col2:
    ...
    for item in consumption_items:
        ...
        item_cols[2].metric("", f"{item['percentage']}%", label_visibility="collapsed")
        item_cols[3].metric("", f"{item['kwh']} kWh", label_visibility="collapsed")

    # Optional: Bar chart for consumption percentages
    # consumption_df = pd.DataFrame(consumption_items).set_index('name')
    # st.bar_chart(consumption_df['percentage']) # Uncomment to show chart


# --- Column 3: Optimization & User ---
with col3:
    st.subheader("💡 Optimization Insights")
    # Removed Lottie, using only subheader icon
    # Display insights using styled boxes
...
# Optional: Add a slight delay to simulate real-time feeling on interaction (if desired)
# time.sleep(0.1)
# st.experimental_rerun() # Use rerun sparingly